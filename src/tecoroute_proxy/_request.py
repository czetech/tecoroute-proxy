...
class ProxyRequest:
    ...
    def __init__(self, request: Request, origin: str = ORIGIN) -> None:
        ...
        removed_headers = {}
        for header_key in ("Content-Length", "Cookie", "Host"):
            value = client_headers.pop(header_key, None)
            if value is not None:
                removed_headers[header_key] = value
        logs += (
            (DEBUG, f"Client headers {dict(client_headers)}"),
            (DEBUG, f"Removed client headers {removed_headers}"),
        )
